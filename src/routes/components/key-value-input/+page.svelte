	...
	<KeyValueInput label="Large" size="large" value={[{ key: 'mode', value: 'preview' }]} />
		...
		<ComponentCard description="Limited to a maximum of 3 rows">
			<div class="w-full max-w-md">
				...
				{#if value3?.length}
					<p class="text-neutral/70 mt-2 text-xs">{value3.length} / 3 rows</p>
				{/if}
			</div>
		</ComponentCard>
		...
		<ComponentCard description="Convert the pairs to an object with Object.fromEntries">
			<div class="w-full max-w-md">
				...
				{#if value4?.length}
					<pre
						class="border-neutral-muted text-neutral/70 mt-2 overflow-auto rounded border p-2 text-xs">{JSON.stringify(
