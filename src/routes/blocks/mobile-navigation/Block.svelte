...
<Stack align="center">
	<Card class="w-full max-w-xl overflow-hidden" variant="outline" density="small" showBorders>
		{#snippet header()}
			<Stack orientation="horizontal" align="center" justify="between" gap="md" wrap="wrap">
				...
				<Button prefix={plusIcon} size="small" color="primary" onclick={() => (activeView = 1)}>
					New project
				</Button>
			</Stack>
		{/snippet}
		...
		<Stack class="min-h-80" gap="lg">
			{#if activeView === 0}
				<Stack gap="xs">
					...
					<p class="text-neutral/60 text-sm">Three priorities are ready for your attention.</p>
				</Stack>

				<Stack gap="md">
					<Card variant="soft" density="small">
						<Stack orientation="horizontal" align="center" justify="between" gap="md">
							<Stack gap="xs">
								<p class="text-neutral font-medium">Launch onboarding</p>
								<p class="text-neutral/60 text-xs">Design review · 10:30 AM</p>
							</Stack>
					...
					<Card variant="soft" density="small">
						<Stack orientation="horizontal" align="center" justify="between" gap="md">
							<Stack gap="xs">
								...
								<p class="text-neutral/60 text-xs">6 comments from the team</p>
							</Stack>
					...
					<Card variant="soft" density="small">
						<Stack orientation="horizontal" align="center" justify="between" gap="md">
							<Stack gap="xs">
								...
								<p class="text-neutral/60 text-xs">Draft saved 18 minutes ago</p>
							</Stack>
			...
			{:else if activeView === 2}
				<Stack gap="lg">
					<Stack gap="xs">
						...
						<p class="text-neutral/60 text-sm">The latest changes across your workspace.</p>
					</Stack>
					<Stack gap="md">
						<Card variant="soft" density="small">
			...
			{:else}
				<Stack gap="lg">
					<Stack orientation="horizontal" align="center" gap="lg">
						<div
							class="bg-primary-muted text-primary-readable flex size-12 shrink-0 items-center justify-center rounded-full"
						>
							{@render userIcon()}
						</div>
						<Stack gap="xs">
							<h2 class="text-neutral text-xl font-semibold">Maya Chen</h2>
							<p class="text-neutral/60 text-sm">Product designer · Lisbon</p>
						</Stack>
					</Stack>
					<Card variant="soft" density="small">
						<Stack gap="sm">
							<p class="text-neutral text-sm font-medium">Workspace plan</p>
							<p class="text-neutral/60 text-sm">Pro · 12 collaborators</p>
						</Stack>
					</Card>
				</Stack>
			{/if}
